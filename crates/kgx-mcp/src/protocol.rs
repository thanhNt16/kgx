    ...
    let result = match method {
        ...
        "tools/call" => {
            ...
            match dispatch(root, name, args).await {
                Ok(out) => json!({"content": [{"type": "text", "text": serde_json::to_string(&out).unwrap_or_default()}]}),
                Err(e) => return Ok(json!({"jsonrpc":"2.0","id":id,"error":{"code":-32603,"message":e.to_string()}})),
            }
        }
